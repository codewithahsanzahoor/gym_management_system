...
export default function Topbar({ role }: TopbarProps) {
  ...
  return (
    <header className="fixed top-0 right-0 h-16 bg-surface/50 backdrop-blur-md flex justify-between items-center ml-64 px-8 w-[calc(100%-16rem)] z-40 border-b border-outline-variant/10">
      <div className="flex items-center flex-1 max-w-xl">
        <div className="relative w-full group">
          <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors">search</span>
          <input
            className="w-full bg-surface-container-low border-none focus:ring-1 focus:ring-primary/40 rounded-lg pl-10 pr-4 py-2 text-sm text-on-surface placeholder:text-slate-500 transition-all"
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-3 pl-4 border-l border-outline-variant/20">
          <div className="text-right">
            <p className="text-xs font-bold text-slate-100 uppercase tracking-tight">{user.name}</p>
